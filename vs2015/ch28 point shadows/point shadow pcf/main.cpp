...
void renderObjs(const Shader& shader) {
	glm::mat4 model;
...
void renderDepthMap(const Shader& shader) {
	const GLuint DEPTHMAP_WIDTH = 1024, DEPTHMAP_HEIGHT = 1024;
	if (depthFbo == 0)
	{
		...
		glBindTexture(GL_TEXTURE_CUBE_MAP, depthTexID);
	...
	// calc view matrix
	glm::mat4 projection = glm::perspective(glm::radians(90.0f), (GLfloat)DEPTHMAP_WIDTH / (GLfloat)DEPTHMAP_HEIGHT, near_plane, far_plane);
	std::vector<glm::mat4> shadowTransforms;
	shadowTransforms.push_back(projection * glm::lookAt(lightPos, lightPos + glm::vec3(1.0, 0.0, 0.0), glm::vec3(0.0, -1.0, 0.0)));
	shadowTransforms.push_back(projection * glm::lookAt(lightPos, lightPos + glm::vec3(-1.0, 0.0, 0.0), glm::vec3(0.0, -1.0, 0.0)));
	shadowTransforms.push_back(projection * glm::lookAt(lightPos, lightPos + glm::vec3(0.0, 1.0, 0.0), glm::vec3(0.0, 0.0, 1.0)));
	shadowTransforms.push_back(projection * glm::lookAt(lightPos, lightPos + glm::vec3(0.0, -1.0, 0.0), glm::vec3(0.0, 0.0, -1.0)));
	shadowTransforms.push_back(projection * glm::lookAt(lightPos, lightPos + glm::vec3(0.0, 0.0, 1.0), glm::vec3(0.0, -1.0, 0.0)));
	shadowTransforms.push_back(projection * glm::lookAt(lightPos, lightPos + glm::vec3(0.0, 0.0, -1.0), glm::vec3(0.0, -1.0, 0.0)));

	shader.use();
	for (GLuint i = 0; i < 6; i++)
	{
		glUniformMatrix4fv(glGetUniformLocation(shader.program, ("depthMatrices[" + std::to_string(i) + "]").c_str()), 1, GL_FALSE, glm::value_ptr(shadowTransforms[i]));
	}
	glUniform3fv(glGetUniformLocation(shader.program, "u_lightPos"), 1, glm::value_ptr(lightPos));
...
void renderScene(const Shader& shader) {	
	...
	glUniform3fv(glGetUniformLocation(shader.program, "u_lightColor"), 1, glm::value_ptr(lightColor));
	glUniform3fv(glGetUniformLocation(shader.program, "u_lightPos"), 1, glm::value_ptr(lightPos));
	glUniform3fv(glGetUniformLocation(shader.program, "u_viewPos"), 1, glm::value_ptr(camera.Position));
	glUniform1f(glGetUniformLocation(shader.program, "far_plane"), far_plane);

	glActiveTexture(GL_TEXTURE0);
	glUniform1i(glGetUniformLocation(shader.program, "texObj"), 0);
	glBindTexture(GL_TEXTURE_2D, sceneTexID);

	glActiveTexture(GL_TEXTURE1);
	glUniform1i(glGetUniformLocation(shader.program, "texDepth"), 1);	
	glBindTexture(GL_TEXTURE_CUBE_MAP, depthTexID);
...
#pragma region "resource"
	Shader depthmapShader("depthmap.vs", "depthmap.frag", "depthmap.gs");
	//Shader quadShader("quad.vs", "quad.frag");
	Shader sceneShader("scene.vs", "scene.frag");
	if (sceneTexID == 0)
	{
		sceneTexID = loadTexture("wood.png", false, true);
	}
#pragma endregion
