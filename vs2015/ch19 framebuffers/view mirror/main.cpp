...
#pragma endregion
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//view = glm::rotate(view, glm::radians(180.0f), camera.Right);		
		camera.Yaw += 180.0f; // Turn the camera's yaw 180 degrees around
		camera.Pitch += 180.0f; // Turn the camera's pitch 180 degrees around
		camera.ProcessMouseMovement(720.0f, 0, false); // Call this to make sure it updates its camera vectors (should probably create an update function ;)), Note that we removed the pitch constrains for this specific case in the camera class via a boolean (otherwise we can't reverse camera's pitch values)
		view = camera.GetViewMatrix();
		camera.Yaw -= 180.0f; // Reset it back to what it was
		camera.Pitch -= 180.0f;
		camera.ProcessMouseMovement(-720.0f, 0);
